I took this from some other app and global replaced the name. Plz update this and make it generic!

I tell sonnet/opus this for adding tkinter GUI to what was a cli.

# Contributing to the Tkinter GUI

This document explains the architecture, design decisions, and conventions
behind metametameta's tkinter GUI so that contributors can extend or modify it
confidently.
...
```bash
uv run metametameta gui        # subcommand
uv run metametameta --gui      # top-level flag
uv run metametameta-gui        # gui-scripts entry point (pyproject.toml)
```

All three converge on `metametameta.gui.app:launch_gui`.
...
def _fetch(scope_str):
    ...
    from metametameta.services import read_current_report
...
1. Add a sidebar entry in `metametametaApp._build_ui`:
...
```python
with patch("metametameta.services.read_current_report",
