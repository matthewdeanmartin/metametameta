...
# 	@find metametameta -name '*.pyc' -exec rm -f {} + || true
#	@find metametameta -name '*.pyo' -exec rm -f {} + || true
#	@find metametameta -name '__pycache__' -exec rm -fr {} + || true
...
.PHONY:
docker:
	docker build -t metametameta -f Dockerfile .

check_docs:
	$(VENV) interrogate metametameta --verbose
