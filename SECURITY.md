# Security Policy for `metametameta`
...
COPY --chown=appuser:appuser metemetameta/ ./metemetameta/

# Set the entrypoint to run the tool
ENTRYPOINT ["metemetameta"]
CMD ["--help"]
```
...
docker run --rm -v "$(pwd):/data" metemetameta /data
```
