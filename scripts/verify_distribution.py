...
PACKAGE_ROOT = PROJECT_ROOT / "metametameta"
...
def verify(directory: Path) -> None:
    ...
    # This project's hatch sdist config ships only the package (no tests), so the
    # sdist is checked against the same file set as the wheel.
    require_files(wheel, package_files, wheel_actual)
    require_files(sdist, package_files, sdist_actual)
    ...
    print(
        ...
        f"{len(test_python_files)} test modules exist locally (not shipped)."
    )
