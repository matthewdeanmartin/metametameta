...
# readonly SCRIPT_NS="metametameta_proc"
readonly SRC_DIR="./metametameta"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/metametameta"
readonly OUTPUT_MD_BASENAME="metametameta.flat.md"

# Copy directory safely
metametameta_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/metametameta
metametameta_proc::run_strip_docs() {
  local dir
...
metametameta_proc::flatten_repo() {
...
# in the generated Markdown (metametameta.flat.md). Other code fences remain untouched.
metametameta_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  metametameta_proc::copy_to_tmp
  metametameta_proc::run_strip_docs
  metametameta_proc::confirm "Run coderoller-flatten-repo?"
  metametameta_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  metametameta_proc::clean_flat_md_trailing_double_hash
  metametameta_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  metametameta_proc::cleanup_tmp
}
